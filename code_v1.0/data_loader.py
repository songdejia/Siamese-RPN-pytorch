...
# @Last Modified time: 2018-11-21 17:05:12
import sys
import os
...
class Anchor_ms(object):
    ...
    def diff_anchor_gt(self, gt):
        ...
        diff[:,2] = np.log((gt[2])/(anchors[:,2] + eps))
        diff[:,3] = np.log((gt[3])/(anchors[:,3] + eps))
        return diff
...
class TrainDataLoader(object):
    ...
    def _pick_img_pairs(self, index_of_subclass):
        ...
        detection_index= random.choice(range(1, self.max_inter)) + template_index
